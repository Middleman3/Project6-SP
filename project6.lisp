...
;;; list, until either you discover it's reachable, or nothing else is moving.
  ...
  (dolist (x assoc-list nil)
    (when (and (equalp (operator-name (car x)) (operator-name from))
	       (or (equalp (operator-name (cdr x)) (operator-name to))
...
;;; perhaps you have an existing function which could help here.
  (let ((isBefore nil)
        (r12 (reachable (plan-orderings plan) operator1 operator2))
	(r21 (reachable (plan-orderings plan) operator2 operator1)))
    (if (and r12 (not r21))
	(setf isBefore t))
    (return-from before-p isBefore)))
...
precond is a predicate."
  (dolist (link (plan-links plan))
    (if (and (equal precond (link-precond link)) (string= (operator-name (link-to link)) (operator-name operator)))
...
;;; SPEED HINT.  Test the easy tests before the more costly ones.
  (let ((to (reachable (plan-orderings plan) operator (link-to link)))
	(from (reachable (plan-orderings plan) operator (link-from link))))
    (if (and (not to) (not from))
	(dolist (effect (operator-effects operator))
	  (if (member (cdr effect) (mapcar #'cdr (operator-effects (link-to link))))
  ...
  (let ((threats '())
    ...
    (dolist (link (plan-links plan))
      (let ((tmp-from-preconditions (operator-preconditions (link-from link)))
	    ...
	    (new-threat-from (cons (link-from link) link))
	    (new-threat-to (cons (link-to link) link))
	    )
	(if (or (every (lambda (x) x) (mapcar #'predicateIsOpposite (list preconditions) tmp-from-effects)) (every (lambda (x) x) (mapcar #'predicateIsOpposite (operator-effects to) tmp-to-preconditions)))
	    (if (not (member (list new-threat-from) threats))	
		(setf threats (append threats (list new-threat-from)))
		)
	    )
	(if (or (every (lambda (x) x) (mapcar #'predicateIsOpposite (list preconditions) tmp-to-effects)) (every (lambda (x) x) (mapcar #'predicateIsOpposite (operator-effects to) tmp-from-preconditions)))
	    (if (not (member (list new-threat-to) threats))
		(setf threats (append threats (list new-threat-to)))
		)
	    )
	)
      )
...
(defun operatorThreatening (plan maybe-threatening-operator)
  (let ((threats '())
	(preconditions (operator-preconditions maybe-threatening-operator))
	(effects (operator-effects maybe-threatening-operator)))
    (dolist (link (plan-links plan))
      (let ((tmp-from-preconditions (operator-preconditions (link-from link)))
	    (tmp-from-effects (operator-effects (link-from link)))
	    (tmp-to-preconditions (operator-preconditions (link-to link)))
	...
	(if (or (every (lambda (x) x) (mapcar #'predicateIsOpposite preconditions tmp-from-effects)) (every (lambda (x) x) (mapcar #'predicateIsOpposite effects tmp-to-preconditions)))
	    (if (not (member (list new-threat-from) threats))	
		(setf threats (append threats (list new-threat-from)))
		)
	    )
	(if (or (every (lambda (x) x) (mapcar #'predicateIsOpposite preconditions tmp-to-effects)) (every (lambda (x) x) (mapcar #'predicateIsOpposite effects tmp-from-preconditions)))
	    (if (not (member (list new-threat-to) threats))
		(setf threats (append threats (list new-threat-to)))
		)
	    )
